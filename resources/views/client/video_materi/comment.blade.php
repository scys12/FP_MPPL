    ...
    <div class="row">
      ...
      <div class="col-md-12">
        <div class="card-body border-bottom">
          ...
          <h6 class="card-subtitle mb-2 text-muted">ditanyakan oleh {{$question->user->username}} pada {{$question->created_at}}</h6>
        </div>
      </div>
            ...
            <div style="display: flex;flex-direction:row;justify-content: space-between; align-items:baseline">
              ...
              @if ($comment->user_ids == Auth::user()->id)
                <div class="icon d-flex" style="justify-content: center;align-items:end">
                  ...
                  <a href="{{route('video_materi.comment.update', ['id' => $id, 'question_id' => $question->id, 'comment_id' => $comment->id])}}">
      ...
      <div class="col-md-12 mt-3">
        <div class="card-body border-bottom">
          <form action="{{route('video_materi.comment.insert', ['id' => $id, 'question_id' => $question->id])}}" method="post">
            @csrf
            <h5 class="card-title">Jawaban Anda</h5>
            <textarea id="editor" name="comment"  placeholder="Masukkan Jawabanmu"></textarea>
            <div class="mt-3">
  ...
  <div class="modal-dialog">
      <div class="modal-content">
          ...
          <div class="modal-footer">
              <form action="{{route('video_materi.comment.delete', ['id' => $id, 'question_id' => $question->id])}}" method="post" id="modal-form">
                  @csrf
                  @method('DELETE')
